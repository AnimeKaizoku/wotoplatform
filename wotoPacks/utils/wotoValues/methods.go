...
// (the listener which accpeted this connection).
...
func (c *WotoConnection) ReadBytes() ([]byte, error) {
	...
	// the whole data's length is equat to `count`
	n, err = c.readAllBytes(b)
	if err != nil {
		return nil, err
	} else if n != count {
...
func (c *WotoConnection) WriteBytes(b []byte) (int, error) {
	...
	//n, err := c.writeAllBytes()
	//if err != nil {
	//	return BaseIndex, err
	//} else if n != MAX_FIRST_BYTES {
	//	return BaseIndex, ErrCouldntWriteFirstBytes
	//}
...
func (c *WotoConnection) WriteJson(v interface{}) (int, error) {
	...
	logging.SUGARED.Debug("what we are writing is ", string(b))
...
func (c *WotoConnection) SetRegistere(r func(*WotoConnection)) {
	c.registerer = r
}
